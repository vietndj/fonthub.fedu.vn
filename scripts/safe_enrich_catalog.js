...
function enrichFonts(fonts) {
  let count = 0;
  fonts.forEach(font => {
    const diskList = getDiskFilesForFont(font);
    if (diskList.length > 0) {
      count++;
      const origFiles = Array.isArray(font.files) ? font.files : [];

      const newFiles = diskList.map(df => {
        // Find matching original file if any
        const orig = origFiles.find(of => {
          const oStyle = (of.style || '').toLowerCase();
          const oName = (of.filename || of.source_filename || '').toLowerCase();
          return oStyle === df.style.toLowerCase() || oName.includes(df.style.toLowerCase().replace(/\s+/g, ''));
        });
        return {
          filename: df.file,
          source_filename: orig ? (orig.source_filename || orig.filename) : df.file,
          style: df.style,
          weight: df.weight,
          ext: df.ext,
      ...
      // Preserve any unmapped original files so Drive accounting doesn't lose a single file
      origFiles.forEach(of => {
        const already = newFiles.some(nf => nf.source_filename === (of.source_filename || of.filename));
        if (!already) {
          newFiles.push(of);
        }
      });
      ...
      font.files_count = newFiles.length;

      // Ensure web_font_url points to a valid woff2
  ...
  console.log(`Enriched ${count} fonts.`);
}

const catalogData = JSON.parse(fs.readFileSync(catalogPath, 'utf8'));
enrichFonts(catalogData.fonts);
...
if (Array.isArray(fontsData)) {
  enrichFonts(fontsData);
  fs.writeFileSync(fontsPath, JSON.stringify(fontsData, null, 2), 'utf8');
} else if (fontsData.fonts) {
  enrichFonts(fontsData.fonts);
  fs.writeFileSync(fontsPath, JSON.stringify(fontsData, null, 2), 'utf8');
}

console.log('Finished safe enrichment of catalog.json and fonts.json.');
